...
First column that should be checked is the column that reveals the status of the processed certificate: 

Reject column: This field indicates whether the certificate was rejected or not. 
If the entry is marked as ‘False’ indicating acceptance, it means that the certificate was accepted, and the underlying cause of death was provided. 
If an entry is marked as 'True,' indicating rejection, it signifies that the certificate has been flagged for manual review. The computation process can encounter failures due to various reasons, such as ICD-11 codes not being found in the linearization, typographical errors, implausible coding, system errors, and more. 

Then users should go to the Report column that gives an overview of the steps applied by the rule engine to select the underlying cause of death.
