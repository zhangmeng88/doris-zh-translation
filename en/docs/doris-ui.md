...
**Reject column**: This field indicates whether a certificate has been accepted or rejected. Entries can be marked as true or false. An entry marked as 'False' signifies acceptance, meaning that the certificate was processed and the underlying cause of death was provided. On the other hand, if an entry is marked as 'True', it indicates rejection, and the certificate has been flagged for manual review. Rejection can occur due to various reasons, such as ICD-11 codes not being found in the linearization, typographical errors, implausible coding, system errors, and more.
...
**Warnings column**: This field logs any warnings generated during processing. Warnings serve as alerts for potential issues or anomalies that might not necessarily lead to rejection but should be reviewed and taken into consideration. Addressing these warnings can help improve the overall quality and reliability of the underlying cause of death selection.

**UnderlyingCauseOfDeathComputed**: These fields store computed values related to the underlying cause of death. This information includes textual descriptions, ICD-11 codes and URIs (Unique Resource Identifier) helping users identify the selected underlying cause of death in the format they need. **UnderlyingCauseOfDeathComputedcomplete** takes into account postcoordination combinations. More information can be found [here](https://github.com/ICD-API/electronic-death-certificate-format-tabular)

**Timestamp**: The "Timestamp" field records the time of processing, indicating when the entry was evaluated and the results were generated. It helps in tracking the timeline and comparing different versions of the output file.

**ProcessedCauseOfDeath**: These fields provide information about the processed cause of death codes, URIs, textual representations, entity titles, auto-coding messages, and match levels. The last two fields assist users in understanding the text to code mechanism and the rules applied to determine the cause of death. The **ProcessedCauseOfDeathAutoCodingMessageA** field indicates a message related to the automated coding process for the cause of death. In this specific case, the message suggests that the coding process was successful and resulted in a "GoodMatch" with a score of 1. The "ProcessedCauseOfDeathMatchLevelA" field indicates the level of match achieved during the automated coding process. Both fields provide insights into the accuracy and quality of the automated coding for the cause of death, indicating a satisfactory match in this instance.
More information can be found [here](https://github.com/ICD-API/electronic-death-certificate-format-tabular)
...
By examining these fields, users can gain a comprehensive understanding of the status, processing steps, errors, and warnings associated with the death certificates, enabling them to effectively review and interpret the output file generated by the DORIS tool and use the selected underlying cause of death for producing mortality statistics.
