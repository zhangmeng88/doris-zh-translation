...
**UnderlyingCauseOfDeathComputed columns**: These fields store computed values related to the selected underlying cause of death in textual descriptions and ICD-11 codes and URIs (Unique Resource Identifier). More information can be found [here](https://github.com/ICD-API/electronic-death-certificate-format-tabular)
...
Additional columns are provided to help users in managing the available cuase of death information for their reports and statistics. 

If the input file contains ICD-11 codes only, the DORIS tool will provide the relevant entity titles in the ProcessedCauseOfDeathEntityTitles columns i.e: ProcessedCauseOfDeathEntityTitleA, ProcessedCauseOfDeathEntityTitleB, ProcessedCauseOfDeathEntityTitleC, etc.

 If the input file contains textual descritptions the DORIS tool will provide the matching ICD-11 codes for the provided text along with auto-coding messages, and match levels. These columns assist users in understanding the text to code mode and the rules applied to select the cause of death.

**ProcessedCauseOfDeath columns**: These fields provide information about the processed textual descriptions of the cause of death, as well as processed ICD-11 codes and URIs.

  
**ProcessedCauseOfDeathAutoCodingMessage columns** field is related to the automated coding process for the cause of death provided in textual format. The message suggests that whether the coding process was successful and resulted in a "GoodMatch" with a score of 1, or  The "ProcessedCauseOfDeathMatchLevelA" field indicates the level of match achieved during the automated coding process. Both fields provide insights into the accuracy and quality of the automated coding for the cause of death, indicating a satisfactory match in this instance.
More information can be found [here](https://github.com/ICD-API/electronic-death-certificate-format-tabular)
